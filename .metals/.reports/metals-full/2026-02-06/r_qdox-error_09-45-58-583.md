error id: file://<WORKSPACE>/src/test/java/com/example/ApplicationTests.java
file://<WORKSPACE>/src/test/java/com/example/ApplicationTests.java
### com.thoughtworks.qdox.parser.ParseException: syntax error @[58,9]
...
class ApplicationTests {
    ...
    void testEchoEndpoint() throws Exception {
        ...
        m@@ockMvc.perform(get("/api/hello/Alice"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("Hello, Alice!"))
                .andExpect(jsonPath("$.status").value("success"));
    }
...
QDox parse error in file://<WORKSPACE>/src/test/java/com/example/ApplicationTests.java